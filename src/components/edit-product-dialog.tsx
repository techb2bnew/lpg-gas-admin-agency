                              ...
                              {fields.map((field, index) => (
                                  <div key={field.id} className="flex items-start gap-2 p-3 border rounded-md relative">
                                       <div className={cn("grid grid-cols-1 sm:grid-cols-4 gap-2 flex-1", { "sm:grid-cols-3": isAdmin })}>
                                          ...
                                          <FormField control={form.control} name={`variants.${index}.price`} render={({ field }) => (<FormItem><FormLabel>Price (KSH)</FormLabel><FormControl><Input type="number" placeholder="1100" {...field} disabled={!isAdmin} /></FormControl><FormMessage /></FormItem>)} />
                                          {!isAdmin && (
