                              ...
                              {fields.map((field, index) => (
                                  <div key={field.id} className="flex items-start gap-2 p-3 border rounded-md relative">
                                       <div className={cn("grid grid-cols-1 sm:grid-cols-4 gap-2 flex-1", { "sm:grid-cols-3": isAdmin })}>
                                          ...
                                          <FormField control={form.control} name={`variants.${index}.price`} render={({ field }) => (<FormItem><FormLabel>Price (KSH)</FormLabel><FormControl><Input type="number" min="0" placeholder="1100" {...field} disabled={!isAdmin} onChange={(e) => { const value = e.target.value; if (value === '' || parseFloat(value) >= 0) { field.onChange(e); } }} /></FormControl><FormMessage /></FormItem>)} />
                                          {!isAdmin && (
