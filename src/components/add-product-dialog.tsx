                              ...
                              {fields.map((field, index) => (
                                  <div key={field.id} className="flex items-start gap-2 p-3 border rounded-md relative">
                                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 flex-1">
                                          ...
                                          <FormField control={form.control} name={`variants.${index}.price`} render={({ field }) => (<FormItem><FormLabel>Price (KSH) <span className="text-red-500">*</span></FormLabel><FormControl><Input type="number" step="0.01" min="0" placeholder="1100" {...field} onChange={(e) => { const value = e.target.value; if (value === '' || parseFloat(value) >= 0) { field.onChange(e); } }} /></FormControl><FormMessage /></FormItem>)} />
                                      </div>
